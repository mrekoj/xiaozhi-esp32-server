...
import { createStreamingContext } from './stream-context_en.js?v=0127';
...
export class AudioPlayer {
    ...
    enqueueAudioData(opusData) {
        if (opusData.length > 0) {
            ...
            log('Received empty audio data frame, may be end signal', 'warning');
    ...
    getAudioStats() {
        ...
        return {
            ...
            totalPending: pendingDecode + pendingPlay  // Total pending packet count
        };
    }
    ...
    clearAllAudio() {
        ...
        // Clear receive queue (using clear method to keep object references)
        this.queue.clear();

        // Clear all buffers in stream context
        if (this.streamingContext) {
