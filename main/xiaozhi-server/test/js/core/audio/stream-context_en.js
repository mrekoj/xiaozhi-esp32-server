...
export class StreamingContext {
    constructor(opusDecoder, audioContext, sampleRate, channels, minAudioDuration) {
        ...
        this.playing = false;     // Whether playing
    ...
    // Get pending buffer queue - single thread safe
    ...
    // Get playing decoded PCM queue - single thread safe
    ...
    convertInt16ToFloat32(int16Data) {
        ...
        for (let i = 0; i < int16Data.length; i++) {
            // Convert [-32768,32767] range to [-1,1], using 32768.0 uniformly to avoid asymmetric distortion
    ...
    getPendingPlayCount() {
        ...
        // Calculate scheduled but not played samples (in Web Audio buffer)
    ...
    clearAllBuffers() {
        ...
        // Clear all queues (using clear method to keep object references)
    ...
    async startPlaying() {
        ...
        while (true) {
            ...
            // Continuously play audio from queue, one small chunk at a time
            while (this.playing && this.queue.length > 0) {
                ...
                log(`Scheduling playback of ${currentSamples.length} samples, ~${(currentSamples.length / this.sampleRate).toFixed(2)} seconds`, 'debug');
                ...
                // If queue data insufficient, wait for new data
