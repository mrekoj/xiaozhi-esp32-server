...
export async function initMcpTools() {
    ...
    const defaultMcpTools = await fetch("js/config/default-mcp-tools_en.json").then(res => res.json());
            ...
            <div class="mcp-tool-card">
                ...
                <div class="mcp-tool-info">
                    ...
                    <div class="mcp-tool-info-row">
                        ...
                        <span class="mcp-tool-info-value">${hasMockResponse ? '✅ Configured: ' + JSON.stringify(tool.mockResponse) : '⚪ Using default'}</span>
                    </div>
                </div>
            </div>
        `;
...
function renderMcpProperties() {
    ...
    if (mcpProperties.length === 0) {
        container.innerHTML = '<div style="text-align: center; padding: 20px; color: #999; font-size: 14px;">No parameters yet, click the button below to add parameters</div>';
        return;
    }
    container.innerHTML = mcpProperties.map((prop, index) => `
        <div class="mcp-property-item">
            ...
            <div class="mcp-property-row-full">
                <label class="mcp-small-label">Description</label>
...
function setupMcpEventListeners() {
    ...
    // Ensure panel is expanded by default
