// UI controller module (English version)
import { loadConfig, saveConfig } from '../config/manager.js?v=0127';
import { getAudioPlayer } from '../core/audio/player_en.js?v=0127';
import { getAudioRecorder } from '../core/audio/recorder_en.js?v=0127';
import { getWebSocketHandler } from '../core/network/websocket_en.js?v=0127';
...
class UIController {
    ...
    startAIChatSession() {
        this.addChatMessage('Connected! Let\'s chat~😊', false);
        // Check microphone availability and show error messages if needed
        if (!window.microphoneAvailable) {
            if (window.isHttpNonLocalhost) {
                this.addChatMessage('⚠️ Recording unavailable over HTTP, text chat only', false);
            } else {
                this.addChatMessage('⚠️ Microphone unavailable, please check permissions, text chat only', false);
            }
        }
    ...
    async handleConnect() {
        ...
        try {
            ...
            if (isConnected) {
                ...
                const { checkMicrophoneAvailability } = await import('../core/audio/recorder_en.js?v=0127');
